...
```
![Coefficients at for every level of the autocorrelation wavelet transform and stationary wavelet transform on a unit impulse signal. \label{fig:transforms}](transforms.png)
...
```
![The selected best basis trees for 100 noisy heavysine signals using Joint Bast Basis and Least Statistically Dependent Basis. \label{fig:bestbasis}](bestbasis.png)
...
```
![Heavysine signals with Gaussian noise and denoised signals using joint bast basis\label{fig:denoising}](denoising.png)
...
savefig(p, "ldb.png")
```
![Cylinder-Bell-Funnel class signals and the selected best bases for discriminating the signals\label{fig:denoising}](denoising.png)
